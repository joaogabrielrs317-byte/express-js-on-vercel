...
export default function AdminClipping() {
  ...
  if (loading) return <div className="flex justify-center pt-20"><div className="w-6 h-6 border-2 border-accent-500 border-t-transparent rounded-full animate-spin" /></div>

  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      <div className="flex items-center justify-between mb-10">
        <h1 className="font-display text-2xl font-bold text-ink-950 dark:text-ink-50">Clipping</h1>
        <button onClick={add} className="flex items-center gap-2 font-sans text-sm uppercase tracking-widest bg-ink-950 dark:bg-ink-100 text-ink-50 dark:text-ink-950 px-5 py-3 hover:bg-accent-600 transition-colors">
          <Plus size={14} /> Adicionar
        </button>
      </div>

      {clips.length === 0 && (
        <div className="py-20 text-center border border-dashed border-ink-200 dark:border-ink-700">
      ...
      <div className="flex flex-col gap-6">
        {clips.map(clip => (
          <div key={clip.id} className="border border-ink-200 dark:border-ink-700 p-5">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between pt-1">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={clip.highlight} onChange={e => update(clip.id, 'highlight', e.target.checked)} className="w-4 h-4 accent-accent-500" />
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => remove(clip.id)} className="flex items-center gap-1 font-sans text-xs text-ink-400 hover:text-red-500 transition-colors">
                    <Trash2 size={13} /> Remover
                  </button>
                  <button onClick={() => save(clip)} disabled={saving === clip.id} className="flex items-center gap-2 font-sans text-xs uppercase tracking-widest bg-ink-950 dark:bg-ink-100 text-ink-50 dark:text-ink-950 px-4 py-2 hover:bg-accent-600 transition-colors disabled:opacity-50">
